...
MassLookup::MassLookup() {
  ...
  if(massfile.is_open()) {
	...
	std::cerr<<"Unable to initialize mass table!"<<std::endl;
  }
}
...
double MassLookup::FindMass(int Z, int A) {
  ...
  if(data == massTable.end()) {
    std::cerr<<"Unable to find mass in table! Returning 0..."<<std::endl;
    return 0;
  }
  return data->second;
}
...
std::string MassLookup::FindSymbol(int Z, int A) {
  ...
  if(data == elementTable.end()) {
	std::cerr<<"Unable to find symbol in table! Returning empty string..."<<std::endl;
	return "";
  }
